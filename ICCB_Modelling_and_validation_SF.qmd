...
abstract: |
  ...
  6. Fit a Generalised Linear Model (GLM) to the data based on current climatic conditions.
  7. Fit a Random Forest Model to the data based on current climatic conditions.
...
Choosing background points to sample the availability of different environmental conditions is an important step in presence-only modelling.
...
covs <- rast("Data/Environmental_variables/SEQ_current_bioclim.tif")
...
## Step 1. Load koala presences and background points

Here, we call our They are loaded as spatvectors, but we also want them as dataframes for model input requirements.
...
## Step 2. Environmental covariate selection

First, we load the rasters describing the current environmental conditions. We formatted these rasters in the same way as the Koala data, so that they are all in the same projection and extent. We did this in the script: 'ICCB_Environmental_data.qmd'
...
covs_current <- crop(covs_current, SEQ_extent.vect) # Mask to SEQ extent
covs_current <- mask(covs_current, SEQ_extent.vect) # Mask to SEQ extent

```

## Add human footprint layer
...
human_footprint <- crop(human_footprint, SEQ_extent.vect) # Mask to SEQ extent
human_footprint <- mask(human_footprint, SEQ_extent.vect) # Mask to SEQ extent
plot(human_footprint, main = "Human Footprint Index masked to SEQ extent")

```

### Option 1. Narrow down potential covariates based on ecological knowledge

For this example, we had advice from scientists at CSIRO (<link>) who conducted an expert elicitation to gather a set of potential covariates that are likely to be important for koalas. This is an example of one approach for deciding what covariates are candidates for inclusion in your model. We use this expert knowledge to filter out the key bioclim variables.
...
# plot(human_footprint, main = "Human Footprint Index") # Plot human footprint layer)
human_footprint_resampled <- resample(human_footprint, covs_current_expert, method = "bilinear")
# plot(human_footprint_resampled, main = "Human Footprint Index Resampled")

covs_current <- c(covs_current, human_footprint_resampled) # Add human footprint layer
covs_current_expert <- c(covs_current_expert, human_footprint_resampled) # Add human footprint layer
...
train_PB_covs <- train_PB_covs %>% drop_na() # Remove any rows with NA values
...
## Step 4. Check correlation and multicollinearity of covariates
...
## Step 5. Exploration of the koala presence and background data
...
# Check the model results
summary(glm_model_1_bias)

# These response curves don't look very helpful
dismo::response(glm_model_1_bias)

```

```{r}

layerCor(covs_current_expert, fun = "pearson")
...
### Model 1 - without the bias correction

```{r}

covs_current_expert_noHF <- terra::subset(covs_current_expert, "cost_hfp2013", negate=TRUE)

# Predict the presence probability across the entire raster extent
predicted_raster_model_1 <- predicts::predict(covs_current_expert_noHF, glm_model_1, type = "response")
...
### Model 1 - with the bias correction

```{r}

# Predict the presence probability across the entire raster extent
predicted_raster_model_1 <- predicts::predict(covs_current_expert, glm_model_1_bias, type = "response")
...
human_footprint_mean <- human_footprint_resampled
values(human_footprint_mean) <- mean(values(human_footprint_resampled), na.rm = TRUE)

covs_current_expert_HFmean <- terra::subset(covs_current_expert, "cost_hfp2013", negate=TRUE)

# Add the human footprint layer with mean values to the covariates
covs_current_expert_HFmean <- c(covs_current_expert_HFmean, human_footprint_mean)

# Predict the presence probability across the entire raster extent
predicted_raster_model_1 <- predicts::predict(covs_current_expert_HFmean, glm_model_1_bias, type = "response")
...
# Specify the model formula - regression
model_formula_bias <- as.formula(Presence ~
                                   BIO5_Max_Temp_Warmest_Month +
                                   BIO6_Min_Temp_Coldest_Month +
                                   BIO12_Annual_Precipitation +
                                   BIO15_Precip_Seasonality +
...
rf_1_bias <- randomForest::randomForest(formula = model_formula_bias,
                                   data = train_PB_covs_thinned,
                                   weights = weight,
                                   ntree = 1000,
                                   sampsize = sample_size,
...
# Variable importance:
importance(rf_1_bias)

# Plot variable importance
varImpPlot(rf_1_bias, type = 1)

# Look at single trees:
head(getTree(rf_1_bias,1,T))
...
# Now, we plot response curves in the same way as we did for GLMs above:
partial_response(rf_1_bias, predictors = train_PB_covs_thinned[,predictors], main='Random Forest')

# Plot inflated response curves:
inflated_response(rf_1_bias, predictors = train_PB_covs_thinned[,predictors], method = "stat3", lwd = 3, main='Random Forest')
...
# Predict the presence probability across the entire raster extent
predicted_raster_RF_1 <- predicts::predict(covs_current_expert_noHF, rf_1, type = "response")
...
# Predict the presence probability across the entire raster extent
predicted_raster_RF_1 <- predicts::predict(covs_current_expert_HFmean, rf_1_bias, type = "response")
...
covs_future <- rast("Data/Environmental_variables/SEQ_future_bioclim.2090.SSP370.tif")
names(covs_future) <- layer_names
covs_future

covs_future <- terra::mask(covs_future, covs_current[[1]]) # Crop to SEQ extent
...
# Add the human footprint layer with mean values to the covariates
covs_future_expert_HFmean <- c(covs_future_expert, human_footprint_mean)
covs_future_expert_HFmean
...
plot(covs_future_expert - covs_current_expert_noHF)
...
# Predict the presence probability across the entire raster extent
predicted_raster_model_2 <- predict(covs_future_expert_HFmean, glm_model_1_bias, type = "response")
...
# Predict the presence probability across the entire raster extent
predicted_raster_RF_1 <- predicts::predict(covs_future_expert_HFmean, rf_1_bias, type = "response", na.rm=TRUE)
