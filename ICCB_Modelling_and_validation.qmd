...
There are several different methods for creating correlation plots which we demonstrate here.
...
### Functions from the package ENMTools 
...
### Custom function using `corrplot` package

Here, we use custom code from an EcoCommons Australia notebook that calls the `corrplot` package to create a correlation plot of the selected covariates.
...
Each of these correlation tests gives the same message, we have some correlated covariates. Particularly our Bio6 and Bio12. 
