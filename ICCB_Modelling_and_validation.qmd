...
plot(
  ...
  main = "Predicted Probability of Presence of Koalas in SEQ - GLM 1"
)
...
plot(
  ...
  range = c(0, 1),  # Set min and max values for the color scale
  main = "Predicted Probability of Presence of Koalas in SEQ - GLM 2"
)
...
presNum <- as.numeric(table(train_PB_covs_scaled$Presence)["1"]) # number of presences
bgNum <- as.numeric(table(train_PB_covs_scaled$Presence)["0"]) # number of backgrounds
weight <- ifelse(train_PB_covs_scaled$Presence == 1, 1, presNum / bgNum) # down-weighting
...
## Random Forest Model 1 - expert covariates
...
plot(
  ...
  range = c(0, 1),  # Set min and max values for the color scale
  main = "Predicted Probability of Presence of Koalas in SEQ - RF 1"
)

```

## Random Forest Model 2 - all covariates
...
rf_2 <- randomForest::randomForest(x = train_PB_covs_thinned[, 1:19],
                                   # y = train_PB_covs_thinned[, "Presence_Factor"], # for classification
                                   y = train_PB_covs_thinned[, "Presence"], # for regression
                                   weights = weight,
                                   ntree = 1000,
                                   sampsize = sample_size,
                                   replace = T, 
...
# Now, we plot response curves in the same way as we did for GLMs above:
partial_response(rf_2, predictors = train_PB_covs_thinned[,layer_names], main='Random Forest')

# Plot inflated response curves:
inflated_response(rf_2, predictors = train_PB_covs_thinned[,layer_names], method = "stat3", lwd = 3, main='Random Forest') 
...
# Predict the presence probability across the entire raster extent
predicted_raster_RF_2 <- predicts::predict(covs_current, rf_2, type = "response", na.rm=TRUE)

# Plot the species distribution raster
plot(
  predicted_raster_RF_2,
  range = c(0, 1),  # Set min and max values for the color scale
  main = "Predicted Probability of Presence of Koalas in SEQ - RF 2"
)
...
# Clear any previous model fits
maxent_model_1 <- NULL

maxent_model_1 <- dismo::maxent(x = select(train_PB_covs_thinned, -c(Presence, x, y)),
...
# Current prediction
pred_current_maxent <- predicts::predict(maxent.mod, covs_current, args = "doclamp=false")
...
# Predict the presence probability across the entire raster extent
predicted_raster_RF_2 <- predicts::predict(covs_future, rf_2, type = "response", na.rm=TRUE)
