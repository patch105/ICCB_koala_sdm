...
In our case, we have will use a covariate for the Global Human Footprint (HFP) index, which is a measure of human impact on the environment. This is a raster layer with continuous (percentage values) that we will use to account for sampling bias in our model.
...
human_footprint <- terra::mask(human_footprint, covs_current_expert[[1]]) # Mask to SEQ extent
...
We use a function from `flexsdm` for this.

```{r}

# train_PB_covs <- sdm_extract(
#   data = pr_bg,
#   x = "x",
#   y = "y",
#   env_layer = covs_current_expert, # Raster with environmental variables
...
PB_covs <- terra::extract(
  x = covs_current_expert,   # Raster with environmental variables
...
ecospat::ecospat.cor.plot(covs_current_expert)
...
# Step 5. Fit our first model: A generalised linear model (GLM)
...
## GLM Model 1 - expert variables with linear terms
...
# Step 6. Fit our second model: A random forest
